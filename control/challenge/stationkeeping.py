...
def getBoxDist(boxCoords):
    ...
    BL2Boat = standardcalc.distBetweenTwoCoords(gVars.currentData[sVars.GPS_INDEX], boxCoords[3]) #bottom left to boat
...
def skrun(boxCoords, wayPtCoords, spdList, meanSpd):
    ...
    while (((datetime.now() - gVars.taskStartTime).seconds < 300) and (gVars.kill_flagSK == 0)):
        ...
        if (exiting == 0):
            gVars.logger.info("WPNSTUFF. Current waypoint lat: " + str(wayPtCoords[gVars.SKCurrentWaypnt].lat) + ". Current waypoint long: " + str(wayPtCoords[gVars.SKCurrentWaypnt].long) + ". current GPS lat: " + str(gVars.currentData[sVars.GPS_INDEX].lat) + ". current GPS long: " + str(gVars.currentData[sVars.GPS_INDEX].long))
            if (standardcalc.isWPNoGoAWA(gVars.currentData[sVars.AWA_INDEX],gVars.currentData[sVars.HOG_INDEX], wayPtCoords[gVars.SKCurrentWaypnt], gVars.currentData[sVars.SOG_INDEX], gVars.currentData[sVars.GPS_INDEX])):
    ...
    def skrun(self, boxCoords, wayPtCoords, spdList, meanSpd):
        ...
        while (((datetime.now() - gVars.taskStartTime).seconds < 300) and (gVars.kill_flagSK == 0)):
            ...
            if (exiting == 0):
                if (standardcalc.isWPNoGo(gVars.currentData.awa,gVars.currentData.hog, wayPtCoords[gVars.SKCurrentWaypnt], gVars.currentData.sog, gVars.currentData.gps_coord)):
                    gVars.logger.info("The boat is sailing upwind. Changing current waypoint.")
                    gVars.SKCurrentWaypnt = (gVars.SKCurrentWaypnt + 1) % 4
                    gVars.logger.info("The current waypoint is " + str(gVars.SKCurrentWaypnt) + ". 0 means top, 1 means right, 2 means bottom, 3 means left")
                    gVars.kill_flagPTP = 1
