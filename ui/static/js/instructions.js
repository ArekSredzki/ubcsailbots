...
function updateWaypointDataDisplayTable()
{
	...
	'<tr id="waypointRow$NUM" class="waypointRow"><td style="background:#5f9be3;color:#fff;">Wpt $NUM</td><td>lat:</td>' +
	        		...
	        		'<td>type:</td>' +
	        		'<td><select id="waypointtype$NUM" onchange="updateWaypoints($NUM)"><option value="none"></option><option value="pointToPoint">Point to Point</option><option value="roundbuoy">Round Buoy</option><option value="ld_start_finish">ld Start Finish</option><option value="ld_first">ld First</option><option value="ld_second">ld Second</option><option value="nav_first">nav First</option><option value="nav_start_port">nav start port</option><option value="nav_start_stbd">nav start stb</option><option value="station_keeping">Station Keeping</option></select></td>' +
